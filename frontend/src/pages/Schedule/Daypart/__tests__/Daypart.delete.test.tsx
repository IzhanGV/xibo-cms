...
describe('Delete Daypart', () => {
  ...
  test('Delete Selected opens the confirmation modal', async () => {
    ...
    expect(within(dialog).getByText('Delete Daypart?')).toBeInTheDocument();
    expect(within(dialog).getByText(mockDaypart.name)).toBeInTheDocument();
  });

  // ---------------------------------------------------------------------------
  // Clicking Cancel closes the modal without calling the API.
  // ---------------------------------------------------------------------------
  test('Cancel closes the modal without calling deleteDaypart', async () => {
    const user = userEvent.setup();
    renderDaypartPage();
  ...
  // ---------------------------------------------------------------------------
  // Clicking "Yes, Delete" on success calls deleteDaypart and closes modal.
  // ---------------------------------------------------------------------------
  test('successful delete calls deleteDaypart and closes the modal', async () => {
    const user = userEvent.setup();
    vi.mocked(deleteDaypart).mockResolvedValueOnce(undefined);
    ...
    expect(deleteDaypart).toHaveBeenCalledTimes(1);
  ...
  test('Yes, Delete button is disabled with loading label while deletion is in progress', async () => {
    ...
    vi.mocked(deleteDaypart).mockReturnValue(controlledPromise);
  ...
  test('failed delete keeps the modal open and shows an error', async () => {
    const user = userEvent.setup();
    vi.mocked(deleteDaypart).mockRejectedValueOnce(new Error('Cannot delete daypart'));
